...
A from-scratch implementation of a two-layer fully connected network in PyTorch — no autograd, no `nn.Module`. Every gradient is derived analytically and computed explicitly, following the backpropagation equations in [`docs/report.pdf`](docs/report.pdf).
